...
final class HomeViewModel {
    ...
    private(set) var trendingGames: [TrendingGamesModel] = []
    private(set) var topRatedGames: [GameModel] = []
...
extension HomeViewModel: HomeViewModelInterface {
    ...
    func fetchApiTasks() {
        Task { // child thread
            view?.showProgress()
            async let fetchTrendingGames: Void = fetchTrendingGames()
            async let fetchTopRatedGames: Void = fetchTopRatedGames()
            
            _ = await (fetchTrendingGames, fetchTopRatedGames)
            view?.removeProgress()
        }
    }
